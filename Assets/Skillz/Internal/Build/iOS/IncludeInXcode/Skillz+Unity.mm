...
- (void)didReceiveEvent:(NSString *)eventName
               withData:(NSDictionary *)eventData
{
    NSString *eventDataJSON = @"";
    NSDictionary *eventDict = @{@"eventName": eventName, @"eventData": eventData};
    if ([NSJSONSerialization isValidJSONObject:eventDict]) {
        NSData *dataData = [NSJSONSerialization dataWithJSONObject:eventDict options:0 error:nil];
        eventDataJSON = [[NSString alloc] initWithData:dataData encoding:NSUTF8StringEncoding];
    }

    dispatch_async(dispatch_get_main_queue(), ^{
        [Skillz sendMessageToUnityObject:unitySkillzDelegateName
                           callingMethod:@"OnEventReceived"
                        withParamMessage:eventDataJSON];
    });
}
